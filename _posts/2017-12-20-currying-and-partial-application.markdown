...
In programming terms function application is a regular function call. In FP we like to look at a function and it's arguments as two separate pieces of data which can be combined together.
...
In layman terms we just pass one or more of the arguments to a function (fixing/binding passed arguments) without evaluating the function. What we get as a result is that same function which requires less arguments. We have to pass number of arguments that is less than the total number of arguments (partial application), otherwise the function would get evaluated (regular function call).
...
Boolean => String
// provide Boolean arg, get:
String
{% endhighlight %}

The pipeline illustrates passing single argument at a time, but more than one argument can be passed if needed.
...
In other words we take a regular function and "curry it" to get a function that takes only one argument and returns another function that also takes only one argument, which in turn returns another function ... until the last function returns the final result.
...
### Currying
In Scala you can create a curried function, i.e. perform currying by calling `curried`. In some languages like Haskell all functions are curried to start with. I like Haskell's approach because there is only one approach to deal with and it fits both needs. 

{% highlight scala %}
// Let's define a new variant of this function that is a curried function:

scala> val f_curried = f _ curried
f_curried: Int => (String => (Boolean => String)) = scala.Function3$$Lambda$1074/2097217770@357bc488
...
// Same result as before but we had to use 3! parameter lists.
// That's because after each param list we would get a function until
// the last application of boolean param that evaluates the function and returns final result.

// Let's see what we get if we provide arguments one by one:

scala> f_curried(1)
res2: String => (Boolean => String) = scala.Function3$$Lambda$1183/1598068850@29f3185c

// A function from String parameter to Boolean => String result, where later is a function itself

scala> f_curried(1)("2")
res3: Boolean => String = scala.Function3$$Lambda$1184/1417325106@3fe98084

// A function from Boolean parameter to String result, where later is a final result

scala> f_curried(1)("2")(true)
res4: String = 12true

// Finally we get the result by passing arguments,
// getting back shorter curried functions until those run out of params and get evaluated
{% endhighlight %}

### Uncurrying
TODO: ...
scala> Function.uncurried(fx)
res13: (Int, String, Boolean) => String = scala.Function$$$Lambda$1263/1558198941@10c67c1c
...
// just serves as an example that if we use underscores (variable placeholders) and don't provide
// any parameters to a function it's still the same function - no partial application happened.
// (Technically speaking in Scala this is a way to convert from a method to a function)
...
// Finally let's provide all arguments as a reminder of what we were trying to achieve:
scala> f(1, "2", true)
res10: String = 12true
{% endhighlight %}
...
Great, but how do we implement partial function application without this library ourselves? Let's ignore some details like `__doc__` and `kwargs`. Let's also not use any Python hacks with regards to how functions and arguments are defined. You can check `partial` docs and implementation for more Pythonic and production grade code. Here we try to express ourselves in a basic way that would also work in many other languages.
...
To implement `curry` we need to build a sequence of lambda functions each taking a single argument - this is what `nest_lambdas` does given `n` number of arguments / nesting levels. For each lambda's argument we use `partial` to pass it to our `func` without evaluating it. Once we exhaust all arguments we just call the partially applied function which already has all arguments fixed for it.
